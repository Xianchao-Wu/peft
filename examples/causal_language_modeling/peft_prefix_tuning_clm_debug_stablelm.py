...
def svd(model_name_or_path):
    ...
    for i in range(alen):
        tempi = layers[i].attention.query_key_value.weight
    ...
    for i in range(alen):
        tempi = layers[i].attention.dense.weight
        ui, si, vi = torch.svd(tempi)
        print(model_name_or_path, 
    ...
    for i in range(alen):
        tempi = layers[i].mlp.dense_h_to_4h.weight
        ui, si, vi = torch.svd(tempi)
        print(model_name_or_path, 
                'mlp.dense_h_to_4h', 
                i, sum(si).item(), len(si), sum(si).item()/len(si), si.shape, ui.shape, vi.shape)
    ...
    for i in range(alen):
        tempi = layers[i].mlp.dense_4h_to_h.weight
        ui, si, vi = torch.svd(tempi)
        print(model_name_or_path, 
                'mlp.dense_4h_to_h', 
                i, sum(si).item(), len(si), sum(si).item()/len(si), si.shape, ui.shape, vi.shape)
