...
def bloom_model_postprocess_past_key_value(past_key_values):
    past_key_values = torch.cat(past_key_values) # 24 [2, 8, 16, 30, 64] -> [48, 8, 16, 30, 64]
    total_layers, batch_size, num_attention_heads, num_virtual_tokens, head_dim = past_key_values.shape
    keys = past_key_values[: total_layers // 2] # 前面的24个是属于keys, [24, 8, 16, 30, 64]
    keys = keys.transpose(2, 3).reshape(
        total_layers // 2, batch_size * num_attention_heads, head_dim, num_virtual_tokens
    ) # [24, 128=batch_size*num_head, 64, 30]
    values = past_key_values[total_layers // 2 :] # [24, 8, 16, 30, 64]
    values = values.reshape(total_layers // 2, batch_size * num_attention_heads, num_virtual_tokens, head_dim) # [24, 128=batch_size*num_head, 30, 64]

    return tuple(zip(keys, values)) # 逐层把key和value区分开；每一层的key, value放一起, out[0][0].shape=[128, 64, 30] for key, and out[0][1].shape=[128, 64, 30] for value NOTE
